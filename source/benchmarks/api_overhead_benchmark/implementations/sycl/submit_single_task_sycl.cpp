...
static TestResult run(const SubmitSingleTaskArguments &arguments, Statistics &statistics) {
    ...
    for (auto i = 0u; i < arguments.iterations; i++) {
        ...
        for (auto iteration = 0u; iteration < arguments.numKernels; iteration++) {
            if (arguments.useEnqueueFunctions) {
                sycl::ext::oneapi::experimental::single_task(queue, eat_time);
            } else {
                queue.single_task(eat_time);
            }
        }
...
[[maybe_unused]] static RegisterTestCaseImplementation<SubmitSingleTask> registerTestCase(run, Api::SYCL);
