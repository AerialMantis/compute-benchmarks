    ...
    for (auto i = 0u; i < arguments.iterations; i++) {
        ...
        for (auto iteration = 0u; iteration < arguments.numKernels; iteration++) {
            if (arguments.useEnqueueFunctions) {
                sycl::ext::oneapi::experimental::parallel_for(queue, range, eat_time);
            } else {
                queue.parallel_for(range, eat_time);
            }
        }
...
[[maybe_unused]] static RegisterTestCaseImplementation<SubmitRangeKernel> registerTestCase(run, Api::SYCL);
