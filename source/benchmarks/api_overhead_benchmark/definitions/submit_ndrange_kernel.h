...
struct SubmitNDRangeKernelArguments : TestCaseArgumentContainer {
...
struct SubmitNDRangeKernel : TestCase<SubmitNDRangeKernelArguments> {
    using TestCase<SubmitNDRangeKernelArguments>::TestCase;

    std::string getTestCaseName() const override {
        return "SubmitNDRangeKernel";
    }

    std::string getHelp() const override {
        return "measures time spent in submitting a kernel using an nd-range to a SYCL (or SYCL-like) queue on CPU.";
    }
};
